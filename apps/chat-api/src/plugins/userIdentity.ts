 ...
 *
 * 1. When CF_ACCESS_TEAM_DOMAIN / CF_ACCESS_AUD are set, decode the
 *    `CF-Access-Jwt-Assertion` header (already validated end-to-end by
 *    Cloudflare before the request reaches this server) and use the JWT
 *    `sub` claim as the user identifier.
 * 2. Fall back to the `X-User-Id` request header (useful for local
 *    development and service-to-service calls).
 * 3. Fall back to the CHAT_DEFAULT_USER_ID environment variable.
 *
